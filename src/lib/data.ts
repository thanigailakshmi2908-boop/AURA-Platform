...
export const AGENTS: AgentDef[] = [
  ...
  { id: 'analytics', name: 'Analytics Agent', role: 'Statistics', icon: 'Calculator', color: '#38bdf8', status: 'idle', description: 'Executes statistical processing & aggregations.' },
  ...
  { id: 'monitor', name: 'Monitoring Agent', role: 'Drift', icon: 'Activity', color: '#3b82f6', status: 'idle', description: 'Evaluates data drift and system errors.' },
...
export const RAG_DOCS: RagDoc[] = [
  { id: 'd1', name: 'Q2 Financial Report.pdf', type: 'PDF', size: '3.4 MB', chunks: 142, status: 'indexed', confidence: 0.94 },
  { id: 'd2', name: 'Data Governance Policy.pdf', type: 'Policy', size: '880 KB', chunks: 56, status: 'indexed', confidence: 0.97 },
  ...
  { id: 'd5', name: 'Customer Churn Analysis.pdf', type: 'PDF', size: '2.3 MB', chunks: 201, status: 'indexed', confidence: 0.92 },
];
...
export const PIPELINES: Pipeline[] = [
  { id: 'p1', name: 'Sales Events Stream', source: 'Kafka / sales-events', throughput: '12.4K/s', latency: '42ms', health: 'healthy', records: '1.2B' },
  { id: 'p2', name: 'Customer CDC Pipeline', source: 'Postgres / Spark', throughput: '3.1K/s', latency: '128ms', health: 'healthy', records: '2.1M' },
  { id: 'p3', name: 'IoT Sensor Feed', source: 'MQTT / Kafka', throughput: '48.7K/s', latency: '18ms', health: 'degraded', records: '15B' },
  { id: 'p4', name: 'Transaction Stream', source: 'Kafka / tx-events', throughput: '8.9K/s', latency: '53ms', health: 'healthy', records: '540M' },
  { id: 'p5', name: 'Inventory Sync', source: 'S3 / Spark', throughput: '1.2K/s', latency: '640ms', health: 'down', records: '900K' },
];
...
export const MODELS: ModelRecord[] = [
  { id: 'm1', name: 'Sales Forecast XGBoost', type: 'Supervised', algorithm: 'XGBoost', accuracy: 0.94, f1: 0.91, status: 'production', version: 'v1.4.0' },
  { id: 'm2', name: 'Anomaly Detector IsolationForest', type: 'Unsupervised', algorithm: 'IsolationForest', accuracy: 0.89, f1: 0.86, status: 'production', version: 'v2.0.4' },
  { id: 'm3', name: 'Churn Predictor LightGBM', type: 'Supervised', algorithm: 'LightGBM', accuracy: 0.87, f1: 0.83, status: 'staging', version: 'v1.0.5' },
  { id: 'm4', name: 'Demand ARIMA Forecast', type: 'Time-Series', algorithm: 'ARIMA', accuracy: 0.82, f1: 0.79, status: 'production', version: 'v1.1.2' },
  { id: 'm5', name: 'Fraud Ensemble XGB+RF', type: 'Ensemble', algorithm: 'XGBoost+RF', accuracy: 0.96, f1: 0.94, status: 'experiment', version: 'v0.8.3' },
  { id: 'm6', name: 'KMeans Sensor Clustering', type: 'Unsupervised', algorithm: 'K-Means', accuracy: 0.78, f1: 0.74, status: 'staging', version: 'v0.1.1' },
];
...
export const ALERTS: AlertEvent[] = [
  ...
  { id: 'a4', severity: 'info', title: 'AutoML experiment v0.9.0 completed', source: 'AutoML', time: '12h ago' },
...
export const AUDIT_LOGS: AuditLog[] = [
  ...
  { id: 'l5', user: 'deepak@aura.io', action: 'DEPLOY', resource: 'model:fraud-ensemble', time: '00:15:44', ip: '10.0.1.24' },
];

export const ROLES = ['Admin', 'Analyst', 'Viewer', 'Data Engineer'];
export const PERMISSIONS = ['Overview', 'Copilot', 'Agents', 'RAG', 'Pipelines', 'ML Engine', 'XAI', 'AutoML', 'Mailing', 'Security', 'Reports', 'Alerts', 'API Portal'];
...
export function rndSeries(seed: number, count: number, base: number, variance: number): SeriesPoint[] {
  ...
  return Array.from({ length: count }, (_, i) => {
    const x = months[i % 12] + (count > 12 ? ` '${Math.floor(1 + i / 12)}` : '');
    const y = Math.round(base + (r() - 0.4) * variance);
    return { x, y };
  });
}
