    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Configure your Groq API credentials, GPT-OSS model routing, and cost optimization.</p>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1 max-w-xl">
                When enabled, AURA dynamically routes queries to the most cost-efficient Groq model capable of handling the payload. Simple queries use GPT-OSS 20B; complex reasoning falls back to GPT-OSS 120B.
              </p>
            </div>
          </div>
      ...
      <Card>
        ...
        <div className="px-5 pb-5 space-y-3">
          {[
            { step: '1', title: 'Visit Groq Console', desc: 'Go to console.groq.com and sign into your developer account.', link: 'https://console.groq.com/keys', linkLabel: 'Groq Console Keys' },
            ...
            { step: '5', title: 'Paste It Into AURA', desc: 'Open the "Manage Groq API Key" drawer above, paste your key, and save.' }
          ].map(s => (
            <div key={s.step} className="flex items-start gap-3 aura-glass rounded-xl p-4">
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-cyan-500/10 text-cyan-400 text-xs font-bold">
                {s.step}
              </div>
              <div>
                ...
                {s.link && (
                  <a href={s.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs font-medium text-cyan-400 hover:text-cyan-300 mt-2">
      ...
      <Drawer isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} title="Groq API Key Management" width="sm">
        <div className="space-y-5">
          <div>
            <p className="text-xs text-slate-400 mb-2">Groq API Key</p>
          ...
          <p className="text-[11px] text-slate-600 mt-2">Keys are encrypted at rest with AES-256. Never exposed in client-side code.</p>
        </div>
      </Drawer>
    </div>
  );
